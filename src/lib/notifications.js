...
export function showTimerNotification(timerType, nextTimer = null) {
  const messages = {
    ...
    standing: {
      ...
      body: nextTimer
        ? `Great job! ${nextTimer === "walking" ? "Time for a walk!" : "Ready for the next timer?"}`
        : "All timers complete!"
    },
    walking: {
  ...
  const message = messages[timerType] || {
...
export function notifyTimerComplete(
  ...
  if (visual) {
    const messages = {
      ...
      standing: nextTimer
        ? `Standing complete! Time for ${nextTimer}`
        : "Standing timer complete! 🎉",
