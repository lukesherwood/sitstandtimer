...
describe('SetTime Component', () => {
  ...
  it('start button is enabled when at least one time is set', async () => {
    ...
    // Find the input within the sitting input wrapper
    const sittingWrapper = screen.getByTestId('sitting-input')
    const sittingInput = sittingWrapper.querySelector('[data-testid="number-input"]')
  ...
  it('updates timer store with correct values on submit', async () => {
    ...
    const sittingInput = sittingWrapper.querySelector('[data-testid="number-input"]')
    const standingInput = standingWrapper.querySelector('[data-testid="number-input"]')
    const walkingInput = walkingWrapper.querySelector('[data-testid="number-input"]')
  ...
  it('starts with standing timer when sitting time is not set', async () => {
    ...
    // Only set standing and walking times
    const standingWrapper = screen.getByTestId('standing-input')
    const walkingWrapper = screen.getByTestId('walking-input')
    const standingInput = standingWrapper.querySelector('[data-testid="number-input"]')
    const walkingInput = walkingWrapper.querySelector('[data-testid="number-input"]')
  ...
  it('displays helpful recommendation text', () => {
    ...
    expect(screen.getByText(/recommended that you get up and stand at least every 50 minutes/i))
      .toBeInTheDocument()
  })
})